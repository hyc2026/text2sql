...
from io import open
from .process_sql import get_schema  # tokenize, , get_tables_with_alias, Schema, get_sql
...
class Evaluator(object):
    ...
    def __init__(self, db_dir, kmaps, etype):
        ...
        for db_name in self.kmaps.keys():
            db_path = os.path.join(db_dir, db_name, "tables.json")
            self.db_paths[db_name] = db_path

            with open(db_path, encoding='utf-8') as ifs:
                table_list = json.load(ifs)
                self.schemas[db_name] = Schema(table_list[0])
        ...
        self.scores['partial_scores'] = {'acc': 0, 'rec': 0, 'f1': 0, 'acc_count': 0, 'rec_count': 0}
    ...
    def eval_partial_match(self, pred, gold):
        ...
        gold_total, pred_total, cnt = self.eval_IUEN(pred, gold)
        acc, rec, f1 = get_scores(cnt, pred_total, gold_total)
    ...
    def evaluate_one(self, db_name, gold, predicted):
        ...
        #gold_str = gold.replace('\"', '')
        gold_str = clean_sql_query(gold)
        gold_sql = get_sql(schema, gold_str)

        self.scores["all"]["count"] += 1
        self.scores["easy"]["count"] += 1
        ...
        try:
            pred_str = clean_sql_query(predicted)
    ...
    def eval_nested(self, pred, gold):
        """
        Args:
        ...
        if pred is not None and gold is not None:
            cnt += self.eval_exact_match(pred, gold)
        return [gold_total, pred_total, cnt]

    def eval_IUEN(self, pred, gold):
        """
        Args:

        Returns:
        """
        lt1, pt1, cnt1 = self.eval_nested(pred['intersect'], gold['intersect'])
        lt2, pt2, cnt2 = self.eval_nested(pred['except'], gold['except'])
        lt3, pt3, cnt3 = self.eval_nested(pred['union'], gold['union'])
...
def update_scores_match(scores, exact_score, hardness, partial_scores, partial_types):
    ...
    for type_ in partial_types:
        ...
        if partial_scores[type_]["gold_total"] > 0:
        ...
        if etype in ["all", "match"]:
            ...
            for type_ in partial_types:
                ...
                if scores[level]["partial"][type_]["acc_count"] == 0:
                    ...
                    if level == 'all':
                        scores['partial_scores']['acc'] += scores[level]["partial"][type_]["acc"]
                        scores['partial_scores']['acc_count'] += scores[level]["partial"][type_]["acc_count"]
                ...
                if scores[level]["partial"][type_]["rec_count"] == 0:
                    ...
                    if level == 'all':
                        scores['partial_scores']['rec'] += scores[level]["partial"][type_]["rec"]
                        scores['partial_scores']['rec_count'] += scores[level]["partial"][type_]["rec_count"]
...
if __name__ == "__main__":
    ...
    arg_parser.add_argument('-g', '--gold', dest='gold', default='data/table.json')
    arg_parser.add_argument('-p', '--pred', dest='pred', default='data/pred.sql')
    arg_parser.add_argument('-s', '--schema', dest='schema', default='data/table.json')
    ...
    for type in ['select', 'where', 'group(no Having)', 'group', 'order', 'and/or', 'IUEN', 'keywords']:
        ...
        print("partial acc: {:6f}, partial rec: {:6f}, partial f1: {:6f}".format( \
            part_scores[type]['acc'], part_scores[type]['rec'], part_scores[type]['f1']))
        print('\n')
