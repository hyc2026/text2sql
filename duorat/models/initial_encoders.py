...
class BertEncoder(InitialEncoder):
    ...
    def _forward_segment(
        ...
        # )        
        last_layer_hidden_state, all_hidden_states = self.bert(
            input_a.to(device=device),
            attention_mask=_input_attention_mask,
            token_type_ids=_input_token_type_ids,
            position_ids=_input_position_ids,
    ...
    def forward(
        ...
        # 是否将question、每个列名、每个表名分开encode
        if self.use_segments:
            source_tensors = []
            batch_size = len(input_segments)
            # 每个segment对应batch的一个数据，其内容为将该数据的input分段为question、每一个表名，每一个列名后的结果。
            ...
            (_batch_size, max_src_length) = input_source_gather_index.shape
            assert _batch_size == batch_size
            # 取[CLS]位置的encode结果作为其后面对应的列名/表名的encode结果，question部分不变。
            source = torch.gather(
                output,
                dim=1,
                index=(
...
@registry.register("initial_encoder", "Roberta")
class RobertaEncoder(InitialEncoder):
    def __init__(
            self,
            pretrained_model_name_or_path: str,
            trainable: bool,
