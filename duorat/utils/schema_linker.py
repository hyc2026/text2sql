...
class SpiderSchemaLinker(AbstractSchemaLinker):
    ...
    def question_to_slml(self, question: str, sql_schema: SQLSchema,) -> str:
       # print(self.tokenizer.tokenize_with_raw(question))
    ...
    for n in range(max_n_gram, 0, -1):
        for (start, end), question_n_gram in get_spans(
            ...
            for column_id, column_name in sql_schema.column_names.items():
                if (
                    column_id not in sql_schema.column_to_table
                ...
                #     print(question_n_gram, column_name, match )
                ...
                if match is NO_MATCH:
                    if (table_id is not None and
                        sql_schema.original_column_names[column_id]!='当前时间'):
                        
                        db_content_matches = []
                        db_content_matches = match_dusql_content(
                        ...
                        # if len(db_content_matches):
                        #     print(db_content_matches)
                        ...
                        if db_content_matches:
                            match = VALUE_MATCH
                           # print(question_n_gram, db_content_matches)
        ...
        for (start, end), question_n_gram in get_spans(
            ...
            for table_id, table_name in sql_schema.table_names.items():
                ...
                if match is not NO_MATCH:
                   ...
                   # print(question_n_gram, table_name, match )
                    # Block the sequence
                    if blocking_match:
                        set_tags(
                            tagged_sequence=tagged_question_tokens, start=start, end=end
...
def span_matches_entity(
    ...
    if with_stemming:
        ...
        entity_name_str = entity_name.lower() # 修复大小写不一致无法匹配的问题
