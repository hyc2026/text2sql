...
def match_dusql_content(
    ...
    def _index_column(columnd_identifier: ColumnIdentifier, db_path: str) -> None:
        ...
        #column_content = get_column_content_dusql(columnd_identifier, db_path)

        # index the row and the words in it
        for row in column_content:
            processed_row = pre_process_words(
                words=jieba.cut(row), with_stemming=with_stemming
            )
...
def get_column_content_dusql(column_identifier: ColumnIdentifier, db_path: str) -> List[str]:
    db_file = db_path 
    # print(open(db_file))
    db_content = json.load(open(db_file))
    # print(len(db_content))
    db_content = db_content[0]
    assert db_content['db_id'] == column_identifier.db_id
    column_name = column_identifier.column_name
    table_name = column_identifier.table_name
    table = db_content['tables'].get(table_name)
    try:
        col_index = table['header'].index(column_name)
    except ValueError:
        print(db_file)
        print(table_name)
        print(table['header'])
        col_content = ['智利' for row in table['cell']]
    else:
        col_content = [row[col_index] for row in table['cell']]
